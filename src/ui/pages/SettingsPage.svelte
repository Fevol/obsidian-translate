...
<div class:disable-animations={!$settings.enable_animations}>
  <nav
    class="translator-navigation-bar"
    tabindex="0"
    on:keydown={(e) => {
      if (e.key === "Tab") {
  ...
  >
    {#each tabs as { id, name, icon }, index}
      <div
        class:translator-navigation-item-selected={$settings_tab === id}
        class="translator-navigation-item"
        ...
        on:contextmenu={(e) => {
          if (ALL_SERVICES.includes(id)) {
            ...
            menu.addItem((item) => {
              item
                .setTitle("Hide service")
                .setIcon("eye-off")
                .onClick((e) => {
                  if (!$settings.filtered_services.length) {
                    $settings.filtered_services = ALL_SERVICES.filter(
                      (s) => s !== id,
                    );
                  } else {
                    $settings.filtered_services =
                      $settings.filtered_services.filter((s) => s !== id);
                  }
      ...
      >
        <div style="display: flex">
          <Icon
            {icon}
            class={id === $settings.translation_service
              ? "translator-selected-element"
              : ""}
          />
        </div>
        <div
          class="translator-navigation-item-text"
          class:translator-navigation-selected-item-text={$settings_tab !== id}
        >
          {name}
        </div>
      </div>
  ...
  {#key $settings_tab}
    <div
      in:horizontalSlide={{ duration: 400, delay: 400 }}
