<script lang="ts">
	...
	const hotkeyMapper = (e: KeyboardEvent) => {
		if (current_id !== null && e.key !== "Shift" && e.key !== "Control" && e.key !== "Meta" && e.key !== "Alt") {
			...
			if (active_modifiers.length || e.key.match(/^F\d+$/)) {
				...
				$settings.hotkeys = [...$settings.hotkeys.filter(h => h.id !== current_id), current_hotkey];
			}
...
<div class="hotkey-list-container">
	{#each HOTKEY_ACTIONS as hotkey, idx}
		{@const set_hotkey_idx = $settings.hotkeys.findIndex(h => h.id === hotkey.id)}
		<SettingItem
			name={t(hotkey.id)}
		>
			<div slot="control" class="setting-item-control">
					...
					{:else}
						<span class="setting-hotkey">
							...
							<span class="setting-hotkey-icon setting-delete-hotkey" aria-label="Delete hotkey" on:click={() => {
								$settings.hotkeys = $settings.hotkeys.filter(h => h.id !== hotkey.id);
							}}>
								<Icon icon="cross"/>
							</span>
						</span>
				...
				<span class="clickable-icon setting-add-hotkey-button" class:mod-active={hotkey.id === current_id} aria-label="Customize this command" on:click={() => {
					if (current_id === null) {
