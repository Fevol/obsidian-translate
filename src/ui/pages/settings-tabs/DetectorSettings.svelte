<script lang="ts">
  ...
  if (!$settings.service_settings.fasttext)
    $settings.service_settings.fasttext =
      DEFAULT_SETTINGS.service_settings.fasttext;
  ...
  <div slot="control" class="setting-item-control">
    <button
      ...
      on:click={async () => {
        ...
        let model_result = await requestUrl({
        ...
        detector = await plugin.reactivity.getTranslationService("fasttext");
        if (!detector?.detector) detector.setup_service(models);
        detector.valid = true;
            ...
            async () => {
              if (
                await plugin.app.vault.adapter.exists(
                  `${plugin.app.vault.configDir}/plugins/translate/models/fasttext`,
                )
              )
