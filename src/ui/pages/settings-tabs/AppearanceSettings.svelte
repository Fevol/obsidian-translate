<script lang="ts">
  ...
  let hide_shortcut_tooltips_toggle =
    plugin.app.loadLocalStorage(`hide_shortcut_tooltips_toggle`) || false;
  ...
  $: top_quickbuttons = Array.from($settings.quicksettings_default).map(
    (id) => {
      return {
        value: id,
        text: QUICK_SETTINGS[id as keyof typeof QUICK_SETTINGS].text,
      };
    },
  );
  $: left_quickactions = $settings.left_quickactions_default.map((id) => {
    return {
      value: id,
      text: QUICK_ACTIONS[id as keyof typeof QUICK_ACTIONS].text,
    };
  });
...
<SettingItem name="General appearance settings" type="heading" />
...
<SettingItem name="Translation View Defaults" type="heading" />

<SettingItem name="Change default quicksettings" type="toggle">
  <div slot="control" class="setting-item-control">
    ...
    <Dropdown
      ...
      value=""
      onChange={(value) => {
        $settings.quicksettings_default = [
          ...$settings.quicksettings_default,
          value,
        ];
      }}
...
<SettingItem name="Change default layout">
  <Dropdown
    slot="control"
...
<SettingItem name="Change default <b>left</b> quickactions" type="toggle">
  <div slot="control" class="setting-item-control">
    <ButtonList
      ...
      onClick={(service, index) => {
        ...
        $settings.left_quickactions_default =
          $settings.left_quickactions_default;
      }}
    />
    <Dropdown
...
<SettingItem name="Change default <b>right</b> quickactions" type="toggle">
  <div slot="control" class="setting-item-control">
    <ButtonList
      ...
      onClick={(service, index) => {
        ...
        $settings.right_quickactions_default =
          $settings.right_quickactions_default;
      }}
    />
    <Dropdown
...
<SettingItem name="Hide attribution info" type="toggle">
  <Toggle
    slot="control"
