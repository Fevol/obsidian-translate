<script lang="ts">
  ...
  let bergamot_update_available =
    $bergamot_data.models && bergamotUpdateAvailable();
  let api_key: string = null;
  let info: typeof SERVICES_INFO = {};

  let obfuscate_api_key =
    plugin.app.loadLocalStorage("obfuscate_keys") || false;

  async function changedService() {
    // Set translator to the selected service
    translator = await plugin.reactivity.getTranslationService(
      service,
      old_service,
    );
    ...
    if (info?.requires_api_key)
      api_key = await plugin.reactivity.getAPIKey(
        service,
        $settings.security_setting,
      );
  ...
  function bergamotModelEqual(
    model: LanguageModelData,
    other: LanguageModelData,
  ) {
    return (
      model.locale === other.locale &&
      model.size === other.size &&
      model.files.length === other.files.length &&
      model.dev_from === other.dev_from &&
      model.dev_to === other.dev_to
    );
  }
  ...
  function bergamotUpdateAvailable() {
    return (
      service === "bergamot" &&
      $bergamot_data.models?.some((model) => {
        const other =
          $settings.service_settings.bergamot.downloadable_models.find(
            (x) => x.locale === model.locale,
          );
  ...
  async function updateBergamot() {
    try {
      let updatable_models =
        $settings.service_settings.bergamot.downloadable_models.filter(
          (model) => {
            const other = $bergamot_data.models.find(
              (x) => x.locale === model.locale,
            );
            return other && !bergamotModelEqual(model, other);
          },
      ...
      const binary_prefetch = await requestUrl({
        url: BERGAMOT_WORKER_LOCATION,
        method: "HEAD",
      });
      const original_binary_size = (
        await plugin.app.vault.adapter.stat(binary_path)
      )?.size;
      if (
        original_binary_size !==
        Number(binary_prefetch.headers["content-length"])
      ) {
        const file = await requestUrl({ url: BERGAMOT_WORKER_LOCATION });
        await writeRecursive(
          plugin.app,
          `${plugin.app.vault.configDir}/plugins/translate/models/bergamot/bergamot-translator-worker.wasm`,
          file.arrayBuffer,
        );
        $bergamot_data.binary.size = Number(
          binary_prefetch.headers["content-length"],
        );
      }
          ...
          if (!stats || stats.size !== modelfile.size) {
            ...
            if (file.status !== 200) {
              plugin.message_queue(
                `Failed to update ${t(model.locale)} language model, aborting update`,
              );
              return;
            }
            await plugin.app.vault.adapter.writeBinary(
              path,
              zlib.gunzipSync(file.arrayBuffer).buffer as ArrayBuffer,
            );
          }
        }
        const model_index = current_models.findIndex(
          (x) => x.locale === model.locale,
        );

        // Remove files that are no longer included in the model
        let to_remove = current_models[model_index].files.filter(
          (x) => !model.files.find((y) => y.name === x.name),
        );
        for (let file of to_remove)
          await plugin.app.vault.adapter.remove(
            `${plugin.app.vault.configDir}/plugins/translate/models/bergamot/${model.locale}/${file.name}`,
          );
      ...
      plugin.message_queue(
        `Failed to update Bergamot language models: ${e.message}`,
      );
    }
  }
  ...
  function filterLanguages(languages: any[]) {
    if (
      service in SERVICES_INFO &&
      SERVICES_INFO[service].type === "translation"
    ) {
      selectable_languages = languages
        .filter((locale) => {
          return !$settings.service_settings[
            service
          ].selected_languages.includes(locale);
        })
        .map((locale) => {
      ...
      filtered_languages = $settings.service_settings[
        service
      ].selected_languages
        ...
        .map((locale) => {
          return {
            value: locale,
            text: $all_languages.get(<string>locale) || locale,
          };
        })
        .sort((a, b) => a.text.localeCompare(b.text));

      if (service === $settings.translation_service) {
        if ($settings.filter_mode === "0") $available_languages = languages;
...
{#if UNTESTED_SERVICES.includes(service)}
  <div class="translator-warning-message">
    ...
    <div>
      <b>WARNING:</b>
      {info.display_name} has not been tested, so it is very likely that it does
      not work properly.<br /><br />
      If you encounter any issue, please open an issue over on
      <a
        href="blank"
        on:click={() => {
          openGithubIssueLink(plugin.app, info.display_name, {});
        }}>GitHub</a
...
<SettingItem name="Service set-up" type="heading" />
...
{#if service === "bergamot"}
  <SettingItem
    ...
    type="button"
    notices={[
      !translator?.has_autodetect_capability()
        ? {
  ...
  >
    <div slot="control" class="setting-item-control">
      ...
      <button
        aria-label={bergamot_update_available
          ? "Bergamot update available"
          : "Install"}
        class:svelcomlib-extra={bergamot_update_available}
        class:svelcomlib-success={!bergamot_update_available &&
        ...
        on:click={async () => {
          if (bergamot_update_available) {
            ...
            let binary_result = await requestUrl({
              url: BERGAMOT_WORKER_LOCATION,
            });
            await writeRecursive(
              plugin.app,
              binary_path,
              binary_result.arrayBuffer,
            );
              ...
              async () => {
                if (
                  await plugin.app.vault.adapter.exists(
                    `${plugin.app.vault.configDir}/plugins/translate/models/bergamot`,
                  )
                )
                  await plugin.app.vault.adapter.rmdir(
                    `${plugin.app.vault.configDir}/plugins/translate/models/bergamot`,
                    true,
                  );
                $bergamot_data = {
                ...
                $settings.service_settings[service].validated = null;
                plugin.message_queue(
                  "Successfully uninstalled Bergamot and its language models",
                );
      ...
      <div slot="control" class="setting-item-control">
        ...
        <ButtonList
          ...
          onClick={async (e) => {
            ...
            const model_idx = $bergamot_data.models.findIndex(
              (x) => x.locale === model.locale,
            );
            ...
            plugin.message_queue(
              `Successfully removed ${t(model.locale)} language model`,
            );
          }}
        />
        <Dropdown
          ...
          onChange={async (value) => {
            if (value) {
              const model = $settings.service_settings[
                service
              ].downloadable_models.find((x) => x.locale === value);
              ...
              const version = $bergamot_data.models
                ? $bergamot_data.version
                : $settings.service_settings.bergamot.version;

              // Connection speed in Bps
              let avg_download_speed = navigator.connection.downlink * 1000000;
              let total_size = model.files.reduce(
                (acc, cur) => acc + cur.size,
                0,
              );
              const progress_bar_length = 10;
              let progress_bar = new Notice(
                `Downloading ${model.files.length} files\nProgress:\t\t   [${" ".repeat(progress_bar_length)}]\nRemaining time: ???s`,
                0,
              );
              ...
              try {
                ...
                for (const [index, modelfile] of model.files
                  .sort((a, b) => {
                  ...
                  if (!index)
                    ...
                    avg_download_speed =
                      (avg_download_speed * index) / (index + 1) +
                      modelfile.size / execution_time / (index + 1);

                  if (file.status !== 200) {
                    progress_bar.hide();
                    plugin.message_queue(
                      `Failed to download ${t(model.locale)} language model`,
                    );
                    return;
                  }
                  await writeRecursive(
                ...
                if (progress_bar.noticeEl.isConnected) {
                  progress_bar.setMessage(
                    `Successfully installed ${t(model.locale)} model\nProgress:\t\t   [${"█".repeat(progress_bar_length)}]\nRemaining time: Finished!`,
                  );
                ...
                if (progress_bar.noticeEl.isConnected) {
                  progress_bar.setMessage(
                    `Installation of ${t(model.locale)} model failed\nProgress:\t\t   [${"↯".repeat(progress_bar_length)}]\nRemaining time:\t\t   Failed!\n$Reason: ${e.message}`,
                  );
                } else {
                  plugin.message_queue(
                    `Installation of ${t(model.locale)} model failed\nReason: ${e.message}\nIf you get a 404, try updating the languages using the button below, if that still does not work, open an issue on GitHub`,
                    0,
                  );
                }
              }
...
{#if info.requires_api_key !== undefined}
  <SettingItem
    ...
    notices={[
      ...
      ...(api_key?.endsWith("==")
        ? [{ text: `API key is still encrypted`, type: "error" }]
        : []),
    ]}
  >
    <Input
...
{#if info.requires_host}
  <SettingItem
    ...
    type="text"
    notices={[
      service === "openai_translator"
        ? null
        : { text: "ⓘ You can host this service locally", url: info.local_host },
    ]}
  >
    <div slot="control">
      ...
      onToggle={async () => {
        ...
        if (validation_results.message)
          plugin.message_queue(
            ...
            !validation_results.valid ? 5000 : 3000,
          );
        ...
        if (validation_results.valid && info.standard_languages) {
          $settings.service_settings[service].premium =
            validation_results.premium;
          $settings.service_settings[service].available_languages = (
            await translator.languages()
          ).languages;
          plugin.reactivity.updateAvailableLocales();
        }

        $settings.service_settings[service].validated =
...
<SettingItem name="Translator settings" type="heading" />

<SettingItem
  name="Automatic translate"
  description="Translate text as it is being typed"
  type="toggle"
  notices={service === "bergamot"
    ? []
    : [
        {
...
{#if $settings.service_settings[service].auto_translate}
  ...
  >
    <Slider
      ...
      onChange={(value) => {
        $settings.service_settings[service].auto_translate_interval =
          parseInt(value);
      }}
    />
  </SettingItem>
    ...
    <ButtonList
      ...
      onClick={(locale) => {
        $settings.service_settings[service].selected_languages =
          $settings.service_settings[service].selected_languages.filter(
            (l) => l !== locale,
          );
        filterLanguages(current_available_languages);
      }}
...
>
  <Button
    ...
    onClick={async () => {
      ...
      if (return_values.message) plugin.message_queue(return_values.message);
      if (return_values.languages) {
        if (return_values.languages[0] instanceof Object) {
          $settings.service_settings[service].downloadable_models =
            return_values.languages;
          if (bergamotUpdateAvailable()) bergamot_update_available = true;
  ...
  >
    <Button
      ...
      onClick={async () => {
        ...
        if (output.languages) {
          $settings.service_settings[service].glossary_languages =
            output.languages;
...
{#if info.options}
  <SettingItem name="Service-specific options" type="heading" />
  ...
  {#if info.options.formality}
    <SettingItem
      ...
      type="dropdown"
      notices={[{ text: "Not all languages support formality", type: "info" }]}
    >
      <Dropdown
        slot="control"
  ...
  {#if info.options.profanity_filter}
    ...
    >
      <Dropdown
        slot="control"
        value={$settings.service_settings[service].profanity_filter?.action ??
          "none"}
        options={[
        ...
        onChange={(val) => {
          let profanity =
            $settings.service_settings[service].profanity_filter ?? {};
    ...
    {#if $settings.service_settings[service].profanity_filter?.action === "mark"}
      ...
      >
        <Dropdown
          slot="control"
          value={$settings.service_settings[service].profanity_filter?.marker ??
            "mask"}
          options={[
          ...
          onChange={(val) => {
            $settings.service_settings[service].profanity_filter.marker =
              val.target.value;
