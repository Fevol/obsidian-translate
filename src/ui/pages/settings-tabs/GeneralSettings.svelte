<script lang="ts">
  ...
  import { SERVICES_INFO, SECURITY_MODES } from "../../../constants";
  import { openGithubIssueLink } from "../../../obsidian-util";
  import { ALL_SERVICES } from "../../../types";

  export let plugin: TranslatorPlugin;

  $: selectable_services = ALL_SERVICES.filter(
    (service) => !$settings.filtered_services.includes(service),
  );
  ...
  const example_languages: string[] = [...$all_languages.keys()]
  ...
  function clearAPIKey(service: string, old_mode: string, new_mode: string) {
    if (
      (old_mode === "none" || old_mode === "password") &&
      !(new_mode === "none" || new_mode === "password")
    ) {
      $settings.service_settings[
        service as keyof typeof $settings.service_settings
      ].api_key = undefined;
  ...
  async function updateAPIKeys(old_mode: string, new_mode: string) {
    for (let service in $settings.service_settings) {
      if (
        SERVICES_INFO[service as keyof typeof SERVICES_INFO].requires_api_key
      ) {
        await plugin.reactivity.setAPIKey(
          service,
          new_mode,
          (await plugin.reactivity.getAPIKey(service, old_mode)) || "",
        );
...
>
  <Dropdown
    ...
    options={[
      ...
      { value: "local", text: "Native language" },
    ]}
...
<SettingItem
  ...
  notices={[
    {
      text: `${SECURITY_MODES.find((x) => x.value === $settings.security_setting).info}`,
      type: "info",
    },
  ]}
>
...
>
  <div slot="control" transition:slide class="setting-item-control">
    <ButtonList
      items={$settings.filtered_services.map((service) => {
        return { value: service, text: SERVICES_INFO[service].display_name };
      })}
      icon="cross"
      onClick={(service) => {
        $settings.filtered_services = $settings.filtered_services.filter(
          (x) => x !== service,
        );
    ...
    />
    <Dropdown
      options={selectable_services.map((service) => {
        return { value: service, text: SERVICES_INFO[service].display_name };
      })}
      value=""
      onChange={(value) => {
