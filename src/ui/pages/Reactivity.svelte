...
<script lang="ts">
  ...
  function setAvailableLanguages(
    ...
    const languages =
      plugin.translator.available_languages ||
      $settings.service_settings[translation_service].available_languages;
    if (filter_mode === "1")
      $available_languages = languages.filter((x) =>
        $spellcheck_languages.includes(x),
      );
    else if (filter_mode === "2")
      $available_languages = languages.filter((x) =>
        $settings.service_settings[
          translation_service
        ].selected_languages.includes(x),
      );
    else $available_languages = languages;
  }
   ...
   */
  export function setTranslationService(
    translation_service: string,
    filter_mode: string,
  ) {
    getTranslationService(translation_service, previous_service).then(
      (service) => {
  ...
  function updateLocales() {
    $all_languages.forEach((language, locale) =>
  ...
  function formatLocale(locale: string) {
    ...
    else if ($settings.display_language === "display") language = t(language);
  ...
  export async function getAPIKey(service: string, mode: string) {
    if (mode === "password")
      ...
      return plugin.app.loadLocalStorage(`${service}_api_key`);
    else if (mode === "dont_save")
      return sessionStorage.getItem(`${service}_api_key`);
    else return $settings.service_settings[service].api_key;
  }
  ...
  export async function setAPIKey(service: string, mode: string, key: string) {
    if (mode === "password")
      ...
      plugin.app.saveLocalStorage(service + "_api_key", key);
    else if (mode === "dont_save")
      sessionStorage.setItem(service + "_api_key", key);
    else $settings.service_settings[service].api_key = key;
  }
  ...
  export async function getTranslationService(
    service: string,
    old_service: string = "",
  ): Promise<DummyTranslate | undefined> {
    // Do not attempt to create a service if it does not exist
    if (
      !service ||
      !(service in SERVICES_INFO) ||
      ($settings.filtered_services.length &&
        !$settings.filtered_services.contains(service))
    )
      return undefined;

    let translator: DummyTranslate;

    if (active_services.has(service)) {
      translator = active_services.get(service);
      service_uses.set(service, service_uses.get(service) + 1);
    } else {
      ...
      if (!$settings.service_settings[service])
        $settings.service_settings[service] =
          DEFAULT_SETTINGS.service_settings[service];

      const service_settings = $settings.service_settings[service];
      if (
        $settings.security_setting !== "none" &&
        SERVICES_INFO[service].requires_api_key
      )
        service_settings.api_key = await getAPIKey(
          service,
      ...
      if (service === "google_translate")
        ...
        translation_service = new BergamotTranslate(
          active_services.has("fasttext")
            ? active_services.get("fasttext")
            : await getTranslationService("fasttext", ""),
          plugin,
          $bergamot_data,
        );
        ...
        translation_service.failure_count_watcher.subscribe((failure_count) => {
          ...
          if (failure_count >= 5) {
            ...
            plugin.message_queue(
              `Too many failures: please revalidate ${SERVICES_INFO[service].display_name}`,
              5000,
              true,
            );
          }
      ...
      if (service === "bergamot") {
        ...
        translation_service.available_languages = $bergamot_data.models
          ? ["en", ...$bergamot_data.models.map((m) => m.locale)]
          : ["en"];
        ...
        translation_service.available_languages =
          $settings.service_settings[service].available_languages;
      }
      ...
      active_services.set(service, translation_service);
      translator = translation_service;
      service_uses.set(service, 1);
    }
  ...
  $: {
    if (
      $bergamot_data &&
      Object.values($bergamot_data).some((v) => !(v == null))
    )
      plugin.app.saveLocalStorage("bergamot", JSON.stringify($bergamot_data));
    else localStorage.removeItem(`${plugin.app.appId}-bergamot`);
  }

  $: {
    if (
      $fasttext_data &&
      Object.values($fasttext_data).some((v) => !(v == null))
    )
      plugin.app.saveLocalStorage("fasttext", JSON.stringify($fasttext_data));
    else localStorage.removeItem(`${plugin.app.appId}-fasttext`);
  }
  ...
  function updateSpellcheckerLanguages() {
    $spellcheck_languages = [
      ...new Set(
        plugin.app.vault.config.spellcheckLanguages!.map((x) => {
          return x.split("-")[0];
        }),
      ),
    ];
  ...
  export function filterAvailableServices() {
    ...
    if (Platform.isMobile)
      new_available_services = new_available_services.filter(
        (service) => !SERVICES_INFO[service].desktop_only,
      );
    if ($settings.filtered_services.length)
      new_available_services = new_available_services.filter((service) =>
        $settings.filtered_services.includes(service),
      );
    $available_translator_services = new_available_services.filter(
      (service) => SERVICES_INFO[service].type === "translation",
    );
    $available_detector_services = new_available_services.filter(
      (service) => SERVICES_INFO[service].type === "detection",
    );
  ...
  onMount(async () => {
    if (plugin.app.vault.config.spellcheckLanguages)
      $spellcheck_languages = [
        ...new Set(
          plugin.app.vault.config.spellcheckLanguages.map(
            (x) => x.split("-")[0],
          ),
        ),
      ];
    // Mobile (iOS and Android) do not have spellchecker languages available (assume display language)
    else $spellcheck_languages = [plugin.current_language];
    ...
    plugin.registerEvent(
      plugin.app.vault.on("config-changed", () => {
        if (plugin.app.vault.config.spellcheckLanguages) {
      ...
      for (let service in $settings.service_settings) {
        if (
          $settings.service_settings[service].selected_languages !==
            undefined &&
          !$settings.service_settings[service].selected_languages.length
        ) {
          $settings.service_settings[service].selected_languages =
            $spellcheck_languages;
        }
    ...
    if ($settings.security_setting === "password") {
      for (const [service, service_settings] of Object.entries(
        $settings.service_settings,
      )) {
        if (
          SERVICES_INFO[service].requires_api_key &&
          ...
          if (
            (await aesGcmDecrypt(service_settings.api_key, $password)).endsWith(
              "==",
            )
          ) {
    ...
    if (
      Platform.isMobile &&
      SERVICES_INFO[$settings.translation_service].desktop_only
    ) {
      plugin.message_queue(
        `${toTitleCase($settings.translation_service)} is currently not supported on mobile devices, defaulting to first available service`,
        5000,
        true,
      );
    }
    ...
    if (
      $available_detector_services.contains("fasttext") &&
      !plugin.detector &&
      ($settings.apply_glossary ||
        $settings.service_settings?.fasttext?.default_usage)
    ) {
    ...
    if (
      ...
      loaded_glossaries = await plugin.app.vault.adapter.read(
        `${plugin.app.vault.configDir}/plugins/translate/glossary.json`,
      );
