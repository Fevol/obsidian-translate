...
<script lang="ts">
	...
	export async function getTranslationService(service: string, old_service: string = ''): Promise<DummyTranslate> {
		...
		if (active_services.has(service)) {
			...
			if (service === "google_translate")
				...
				translation_service = new BergamotTranslate(active_services.has("fasttext") ? active_services.get("fasttext") :
	...
	$: {
		if ($bergamot_data && Object.values($bergamot_data).some(v => !(v == null)))
			...
			localStorage.removeItem(`${plugin.app.appId}-bergamot`);
	}
	...
	$: {
		if ($fasttext_data && Object.values($fasttext_data).some(v => !(v == null)))
			...
			localStorage.removeItem(`${plugin.app.appId}-fasttext`);
	}
	...
	onMount(async () => {
		if (plugin.app.vault.config.spellcheckLanguages)
			$spellcheck_languages = [...new Set(plugin.app.vault.config.spellcheckLanguages.map(x => x.split('-')[0]))]
		else
		...
		plugin.registerEvent(plugin.app.vault.on('config-changed', () => {
			if (plugin.app.vault.config.spellcheckLanguages) {
		...
		if (await plugin.app.vault.adapter.exists(`${plugin.app.vault.configDir}/plugins/translate/glossary.json`))
			loaded_glossaries = await plugin.app.vault.adapter.read(`${plugin.app.vault.configDir}/plugins/translate/glossary.json`);
