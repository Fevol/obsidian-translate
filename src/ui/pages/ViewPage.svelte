<script lang="ts">
  ...
  import { Button, Dropdown, Icon, View } from "../components";
  import { TranslatorTextArea } from "../translator-components";
  ...
  import { Scope, Menu } from "obsidian";
  import t from "../../l10n";
  ...
  let parent_view: SvelteComponent | null;
  $: {
    // FIXME: Prefer not to do this, $$ is private API
        ...
        if (hotkey.id === "view-translate") {
          ...
          if (left_buttons.find((button) => button.id === action)) {
            left_view_scope.register(hotkey.modifiers, hotkey.key, () => {
              left_button_actions[action as keyof typeof left_button_actions]();
              return false;
            });
          }
          if (right_buttons.find((button) => button.id === action)) {
            right_view_scope.register(hotkey.modifiers, hotkey.key, () => {
              right_button_actions[
                action as keyof typeof right_button_actions
              ]();
              return false;
            });
          }
  ...
  $: available_language_observer =
    $settings.service_settings[$translation_service].available_languages.length;
  $: selected_languages_observer =
    $settings.service_settings[$translation_service].selected_languages.length;
  ...
  $: $fasttext_data,
    (autodetect_capability =
      translator && translator.has_autodetect_capability());
  ...
  $: {
    ...
    language_from;
    language_to;
    if (auto_translate) autoTranslate();
  }
  ...
  function updateService() {
    plugin.reactivity
      .getTranslationService($translation_service, previous_service)
      .then((service) => {
  ...
  async function translate() {
    ...
    let return_values = await translator.translate(
      text_from,
      language_from,
      selectable_languages.some((x) => x.value === language_to)
        ? language_to
        : "",
      {
        apply_glossary: apply_glossary,
  ...
  async function switchLanguages() {
    if (
      $settings.switch_button_action === "switch-both" ||
      $settings.switch_button_action === "switch-language"
    ) {
      if (language_from === "auto") {
        if (detected_language) {
          [language_from, language_to] = [language_to, detected_language];
          detected_language = undefined;
        } else [language_from, language_to] = [language_to, null];
      } else {
    ...
    if (
      $settings.switch_button_action === "switch-both" ||
      $settings.switch_button_action === "switch-text"
    ) {
  ...
  export async function onResize(width, height) {
    if (view_mode) {
      ...
      if (width_ratio > 1.4) new_layout = "horizontal";
      else if (width_ratio > 1.2) new_layout = "mixed";
      else new_layout = "vertical";
...
<View bind:this={parent_view}>
        ...
        <div class="nav-buttons-container">
          {#each top_buttons as button}
            {#if !top_button_disabled[button]}
              <div
                aria-label={QUICK_SETTINGS[button].tooltip[
                  top_button_states[button]
                ]}
                on:click={top_button_actions[button]}
                class="nav-action-button"
              >
                <Icon
                  icon={QUICK_SETTINGS[button].icon[top_button_states[button]]}
                />
              </div>
            {/if}
          {/each}
  ...
  <div
    slot="view"
    class="translator-view translator-{current_layout}-layout"
    class:disable-animations={!$settings.enable_animations}
  >
    <!-- TODO: Make the field resizable (save data)-->
    <div
      class="translator-column translator-left-column"
      on:mouseenter={() => plugin.app.keymap.pushScope(left_view_scope)}
      on:mouseleave={() => plugin.app.keymap.popScope(left_view_scope)}
    >
      <Dropdown
        value={language_from}
        options={autodetect_capability
          ? [
              detected_language
                ? {
                    value: "auto",
                    text: `Detect Language (${t(detected_language)})`,
                  }
      ...
      <div class="translator-textarea-column">
        ...
        <TranslatorTextArea
          ...
          text={text_from}
          typingdelay={auto_translate &&
            $settings.service_settings[$translation_service]
          ...
          onContextmenu={async (e) => {
            ...
            menu.addItem((item) => {
              item
                ...
                .setSection("general")
                .onClick(async (e) => {
                  text_from = await navigator.clipboard.readText();
                });
            });
            if (selection) {
              menu.addItem((item) => {
                      ...
                      if (detected_language) {
                        ...
                        if (output.detected_languages)
                          glossary.source_language =
                            output.detected_languages[0];
                        else glossary.source_language = plugin.current_language;
                      }
                    } else {
          ...
          <div class="translator-textarea-quickbuttons">
            {#each left_buttons as quick_button}
              <Button
                ...
                icon={QUICK_ACTIONS[quick_button].icon[0]}
                tooltip={QUICK_ACTIONS[quick_button].tooltip[0] +
                  ($hide_shortcut_tooltips ||
                  !$settings.hotkeys.find((x) => x.id.endsWith(quick_button))
                    ?.key
                    ? ""
    ...
    <div class="translator-button-container translator-center-column">
      <button
        ...
        aria-label={`Switch languages around${
          $hide_shortcut_tooltips ||
          !$settings.hotkeys.find((x) => x.id === "view-language-switch")?.key
            ? ""
    ...
    <div
      class="translator-column translator-right-column"
      on:mouseenter={() => plugin.app.keymap.pushScope(right_view_scope)}
      on:mouseleave={() => plugin.app.keymap.popScope(right_view_scope)}
    >
      <Dropdown
        value={language_to}
    ...
    {#if show_attribution}
      <div class="translator-attribution-column">
        <div class="translator-attribution-column-text">
          Using
          <a
            href={services[$translation_service].url}
            target="_blank"
