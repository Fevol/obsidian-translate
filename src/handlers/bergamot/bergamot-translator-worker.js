...
var BergamotModule = (function () {
	var BERGAMOT_VERSION_FULL = "v0.4.4+5ae1b1e";

	return (
		function (BergamotModule) {
			BergamotModule = BergamotModule || {};


			var Module = typeof BergamotModule != "undefined" ? BergamotModule : {};
			...
			function createWasm() {
				...
				async function instantiateAsync() {
					// FIXME: Get rid of this janky-ass code
					...
					if (!await app.vault.adapter.exists(path))
						abort('Could not find bergamot-translator-worker.wasm in the vault');

					let file = await app.vault.adapter.readBinary(path)

					if (!file)
						abort('bergamot-translator-worker.wasm is empty');
			...
			return BergamotModule.ready;
		}
	)
})();
export default BergamotModule;
