			...
			if (t_val && ignored_keys.has(key) || !ignored_keys.has(key)) {
				if (t_val) {
					...
					if (!Array.isArray(t_val) && t_val instanceof Object && s_val instanceof Object) {
						nested_object_assign(
							s_val as Record<string, unknown>,
							t_val as Record<string, unknown>,
							ignored_keys,
						);
					}
				} else {
					...
					if (s_val instanceof Object && !(s_val instanceof Array)) {
						target_dict[key] = Object.fromEntries(
							Object.entries(s_val).filter(([k, v]) => !ignored_keys.has(k)),
						);
					} else {
...
export function getHotKeyString(hotkey: TranslatorHotKey | undefined) {
	if (!hotkey)
		return "";
	return `${
		!hotkey.modifiers.length ? "" : `${hotkey.modifiers.map(mod => modifier_resolver[mod]).join(" + ")} + `
	}${hotkey.key}`;
}
